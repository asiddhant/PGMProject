...
class Loader(object):
    ...
    def load_conll12(self, dataset, parameters):
        
        word_dim = parameters['wrdim']
        pretrained = parameters['ptrnd']
        
        srl_train_data = pkl.load(open(os.path.join(dataset,'train_data.p'),'rb'))
        srl_val_data = pkl.load(open(os.path.join(dataset,'val_data.p'),'rb'))
        srl_test_data = pkl.load(open(os.path.join(dataset,'test_data.p'),'rb'))
    ...
    def load_conll05(self, dataset, parameters):
    ...
    def load_scisrl(self, dataset, parameters):
    ...
    def load_pbsent(self, dataset, parameters):
        ...
        dico_words_train, _, _ = word_mapping(srl_train_data)
        dico_tags, tag_to_id, id_to_tag = tag_mapping(srl_train_data+srl_test_data)

        dico_words, word_to_id, id_to_word = augment_with_pretrained(
                                             dico_words_train.copy(), pretrained,
                                             list(itertools.chain.from_iterable(
                                             [[str(x).lower() for x in s[0]] for s in srl_test_data])))
    ...
    def load_pbpara(self, dataset, parameters):
