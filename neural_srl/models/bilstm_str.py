...
from neural_srl.modules import DecoderLinear
from neural_srl.modules import StructuredAttention


class BiLSTM_STR(nn.Module):
    
    def __init__(self, word_vocab_size, word_embedding_dim, word_hidden_dim, att_dim, tag_to_id, 
                 verb_embedding_dim, cap_embedding_dim, verb_input_dim = 2, cap_input_dim=4, 
                 pretrained=None):
        
        super(BiLSTM_STR, self).__init__()
        ...
        self.structured_attentor = StructuredAttention(word_hidden_dim, att_dim, bidirec=True)
            
        self.decoder = DecoderLinear(2*(word_hidden_dim-att_dim), self.tag_to_ix, input_dropout_p=0.)
    ...
    def forward(self, words, tags, verbs, caps, wordslen, tagsmask, usecuda=True):
        ...
        p_word_features = self.word_encoder(words, verb_features ,cap_features, wordslen)
        word_features = self.structured_attentor(p_word_features)
    ...
    def decode(self, words, verbs, caps, wordslen, tagsmask, usecuda=True, score_only = False):
        ...
        word_features = self.structured_attentor(p_word_features)
        if score_only:
            score = self.decoder.decode(word_features, wordslen, usecuda=usecuda, 
                                        score_only = True)
            return score
        score, tag_seq = self.decoder.decode(word_features, wordslen, usecuda=usecuda)
